...
<p>".$row['status']."</p>
    ...
    echo("<tr><td>no profile available</td></tr>");
...
$sql="SELECT * FROM bookedapt WHERE pid=$id;";
$result=mysqli_query($conn,$sql);
$queryresult=mysqli_num_rows($result);
...
<td>
<h2 class='table-avatar'>
<a href='#' data-bs-target='#patientlist' data-bs-toggle='modal'><img class='avatar avatar-img' src='uploads/".$row['dprofile']."' alt='User Image'></a>
...
$sql="SELECT * FROM trds WHERE userid=$id;";
$result=mysqli_query($conn,$sql);
$queryresult=mysqli_num_rows($result);
...
<td>
<h2 class='table-avatar'>
<a href='#' data-bs-target='#patientlist' data-bs-toggle='modal'><img class='avatar avatar-img' src='uploads/document/".$row['ss']."' alt='User Image'></a>
...
<td>
<h2 class='table-avatar'>
<a href='#' data-bs-target='#patientlist' data-bs-toggle='modal'><img class='avatar avatar-img' src='upload/".$row['username']."' alt='User Image'></a>
...
<p>".$row['status']."</p>
    ...
    echo("<tr><td>no history available</td></tr>");
 ...
 <script>
      ...
      function showPopup() {
      if (confirm("are you sure you want to see users records")) {
        return true;  // User clicked "OK", so proceed to new page
      } else {
        return false;  // User clicked "Cancel", so stay on current page
      }
