...
<i class="feather-chevrons-left"></i>
 ...
 if ($_GET['error']=="acc") {
   echo '<script type="text/javascript">
       window.onload = function () { alert("Dear user acc credentials have been updated"); } 
</script>'; 
...
<div class="row align-items-center">
<div class="col">
<h5 class="card-title">Transaction patient</h5>
</div>
...
<a href='#' data-bs-target='#patientlist' data-bs-toggle='modal'><span class='user-name'>".$row['username']."</span> <span class='text-muted'>".$row['status']."</span></a>
</h2>
</td>
<td><span class='user-name'>".$row['amount']." </span><span class='d-block'></span></td>
<td><span class='user-name'>".$row['trid']." </span><span class='d-block'></span></td>
<td>
<label class='toggle-switch' for='status7'>
<span class='text-muted'>".$row['dated']."</span>
...
transactions done
 ...
 <script>
    function showPopup1() {
      ...
      function showPopup2() {
      if (confirm("Are you sure you want accept transaction?")) {
        return true;  // User clicked "OK", so proceed to new page
      } else {
        return false;  // User clicked "Cancel", so stay on current page
      }
...
<div class="row align-items-center">
<div class="col">
<h5 class="card-title">Doctor Transaction withdrawal</h5>
</div>
<div class="col-auto d-flex">
<div class="form-custom me-2">
<div id="tableSearch" class="dataTables_wrapper"></div>
...
$sql="SELECT * FROM trc where status='requested' ;";
$result=mysqli_query($conn,$sql);
$queryresult=mysqli_num_rows($result);
...
<a  href='includes/withdrawal.php?title=".$row['id']."&?user=".$row['userid']."&?amt=".$row['amount']."' onclick='return showPopup2()' class='btn btn-sm bg-success-light'>
 accept
</a>
<a href='includes/trcreject.php?title=".$row['id']."' onclick='return showPopup1()' class='btn btn-sm bg-danger-light'>
 deny
</a>
...
<div class="card">
 <div class="container">
  <form action="includes/addac.php" method="POST" class="mx-auto">
<div class="col-12 col-md-6 col-lg-4">
<div class="form-group">

<label>Bank account information </label>
<input name="acc" type="text" placeholder="acc" class="form-control">
